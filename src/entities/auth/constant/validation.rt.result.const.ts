...
const validationRTOkResult = {
    'Ok': async (email: string) => console.log(`User with email: ${ email } give new tokens`),
    'Atypical_Device_Data': async (email: string) => {
        console.warn(`User with email: ${ email } was request from new location and user agent.`);
        // Mailer connect;
    }
}
