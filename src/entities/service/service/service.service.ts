...
import { domainNotFound, noAccessToDeactivate, serviceIsAlreadyExist } from '@service/constant';
...
export class ServicesService {
    ...
    async addNewService(servicesData: ServiceData): Promise<string> {
        ...
        try {
            ...
            const isServiceExist = await this.findServiceByNameAndDomainInitial({name, domainId: existDomain.id})
     ...
     * @returns 
     */
    async deactivateService(name: string, domain: string, ip: string, password: string) {
        let result;
        try {
            const { id, host, passhash, salt } = await this.domain.findDomainByName(domain);
            const isCorrectPass = await PasswordHandler.passChecker({ passhash, password, salt })
     ...
     * @param authData
     */
    async findServiceByNameAndDomainInitial(
        data: { 
            name: string,
            domainId?: number,
            domain?: string 
        ...
        try {
            ...
            const service = await this.serviceFinder[domainsInfoKey](name, domainInfoData);
            result = service;
        } catch(err) {
            console.warn(err);
